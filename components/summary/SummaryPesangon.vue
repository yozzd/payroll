<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tPesangon" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryPesangon } from '../../apollo/query/summary';
import { GenPDFSummaryPesangon, GenXLSSummaryPesangon } from '../../apollo/mutation/summary';
...
export default {
    ...
    return {
      ...
      miniSearch: new MiniSearch({
        ...
        storeFields: [
          ...
          'jul', 'agu', 'sep', 'okt', 'nov', 'des', 'tPesangon',
        ],
      }),
    };
  },
  methods: {
    ...
    async genPDFSummary() {
      try {
        ...
        await this.$apollo.mutate({
          mutation: GenPDFSummaryPesangon,
          variables: {
        ...
        window.open(`/summary/${y}/${y}_sum_pesangon.pdf`);
        return true;
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryPesangon: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryPesangon,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
        ...
        if (sStatus) {
          ...
          window.open(`/summary/${y}/${y}_sum_pesangon.xlsx`);
        }
        return true;
      } catch ({ graphQLErrors, networkError }) {
        this.errors = graphQLErrors || networkError.result.errors;
  ...
  apollo: {
    summaryPesangon: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryPesangon;
          this.items = employee;
