<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tICo" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryICo } from '../../apollo/query/summary';
import { GenPDFSummaryICo, GenXLSSummaryICo } from '../../apollo/mutation/summary';
...
export default {
  ...
  methods: {
    ...
    async genPDFSummary() {
      try {
        ...
        window.open(`/summary/${y}/${y}_sum_ins_co.pdf`);
        return true;
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryICo: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryICo,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
        ...
        if (sStatus) {
          ...
          window.open(`/summary/${y}/${y}_sum_ins_co.xlsx`);
        }
        return true;
      } catch ({ graphQLErrors, networkError }) {
        this.errors = graphQLErrors || networkError.result.errors;
  ...
  apollo: {
    summaryICo: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryICo;
          this.items = employee;
