<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tOIncome" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryOIncome } from '../../apollo/query/summary';
import { GenPDFSummaryOIncome, GenXLSSummaryOIncome } from '../../apollo/mutation/summary';
...
export default {
    ...
    return {
      ...
      miniSearch: new MiniSearch({
        ...
        storeFields: [
          ...
          'jul', 'agu', 'sep', 'okt', 'nov', 'des', 'tOIncome',
        ],
      }),
    };
  },
  methods: {
    ...
    async genPDFSummary() {
      try {
        ...
        window.open(`/summary/${y}/${y}_sum_other_income.pdf`);
        return true;
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryOIncome: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryOIncome,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
        ...
        if (sStatus) {
          ...
          window.open(`/summary/${y}/${y}_sum_other_income.xlsx`);
        }
        return true;
      } catch ({ graphQLErrors, networkError }) {
        this.errors = graphQLErrors || networkError.result.errors;
  ...
  apollo: {
    summaryOIncome: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryOIncome;
          this.items = employee;
