<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tThr" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryThr } from '../../apollo/query/summary';
import { GenPDFSummaryThr, GenXLSSummaryThr } from '../../apollo/mutation/summary';
...
export default {
  ...
  methods: {
    ...
    async genPDFSummary() {
      try {
        ...
        window.open(`/summary/${y}/${y}_sum_thr.pdf`);
        return true;
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryThr: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryThr,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
        ...
        if (sStatus) {
          ...
          window.open(`/summary/${y}/${y}_sum_thr.xlsx`);
        }
        return true;
      } catch ({ graphQLErrors, networkError }) {
        this.errors = graphQLErrors || networkError.result.errors;
  ...
  apollo: {
    summaryThr: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryThr;
          this.items = employee;
