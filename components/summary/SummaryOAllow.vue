<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tOAllow" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryOAllow } from '../../apollo/query/summary';
import { GenPDFSummaryOAllow, GenXLSSummaryOAllow } from '../../apollo/mutation/summary';
...
export default {
    ...
    return {
      ...
      miniSearch: new MiniSearch({
        ...
        storeFields: [
          ...
          'jul', 'agu', 'sep', 'okt', 'nov', 'des', 'tOAllow',
        ],
      }),
    };
  },
  methods: {
    ...
    async genPDFSummary() {
      try {
        ...
        window.open(`/summary/${y}/${y}_sum_other_allow.pdf`);
        return true;
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryOAllow: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryOAllow,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
        ...
        if (sStatus) {
          ...
          window.open(`/summary/${y}/${y}_sum_other_allow.xlsx`);
        }
        return true;
      } catch ({ graphQLErrors, networkError }) {
        this.errors = graphQLErrors || networkError.result.errors;
  ...
  apollo: {
    summaryOAllow: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryOAllow;
          this.items = employee;
