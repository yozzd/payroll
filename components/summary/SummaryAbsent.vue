<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tAbsent" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryAbsent } from '../../apollo/query/summary';
import { GenPDFSummaryAbsent, GenXLSSummaryAbsent } from '../../apollo/mutation/summary';
...
export default {
    ...
    return {
      ...
      miniSearch: new MiniSearch({
        ...
        storeFields: [
          ...
          'jul', 'agu', 'sep', 'okt', 'nov', 'des', 'tAbsent',
        ],
      }),
    };
  },
  methods: {
    ...
    async genPDFSummary() {
      try {
        ...
        window.open(`/summary/${y}/${y}_sum_absent.pdf`);
        return true;
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryAbsent: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryAbsent,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
        ...
        if (sStatus) {
          ...
          window.open(`/summary/${y}/${y}_sum_absent.xlsx`);
        }
        return true;
      } catch ({ graphQLErrors, networkError }) {
        this.errors = graphQLErrors || networkError.result.errors;
  ...
  apollo: {
    summaryAbsent: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryAbsent;
          this.items = employee;
