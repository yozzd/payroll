<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tODeduction" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryODeduction } from '../../apollo/query/summary';
import { GenPDFSummaryODeduction, GenXLSSummaryODeduction } from '../../apollo/mutation/summary';
...
export default {
    ...
    return {
      ...
      miniSearch: new MiniSearch({
        ...
        storeFields: [
          ...
          'jul', 'agu', 'sep', 'okt', 'nov', 'des', 'tODeduction',
        ],
      }),
    };
  },
  methods: {
    ...
    async genPDFSummary() {
      try {
        ...
        await this.$apollo.mutate({
          mutation: GenPDFSummaryODeduction,
          variables: {
        ...
        window.open(`/summary/${y}/${y}_sum_other_deduction.pdf`);
        return true;
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryODeduction: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryODeduction,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
        ...
        if (sStatus) {
          ...
          window.open(`/summary/${y}/${y}_sum_other_deduction.xlsx`);
        }
        return true;
      } catch ({ graphQLErrors, networkError }) {
        this.errors = graphQLErrors || networkError.result.errors;
  ...
  apollo: {
    summaryODeduction: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryODeduction;
          this.items = employee;
