<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tOT" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryOT } from '../../apollo/query/summary';
import { GenPDFSummaryOT, GenXLSSummaryOT } from '../../apollo/mutation/summary';
  ...
  methods: {
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryOT: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryOT,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
