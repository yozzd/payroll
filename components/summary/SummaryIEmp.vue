<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tIEmp" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryIEmp } from '../../apollo/query/summary';
import { GenPDFSummaryIEmp, GenXLSSummaryIEmp } from '../../apollo/mutation/summary';
...
export default {
  ...
  methods: {
    ...
    async genPDFSummary() {
      try {
        ...
        window.open(`/summary/${y}/${y}_sum_ins_emp.pdf`);
        return true;
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryIEmp: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryIEmp,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
        ...
        if (sStatus) {
          ...
          window.open(`/summary/${y}/${y}_sum_ins_emp.xlsx`);
        }
        return true;
      } catch ({ graphQLErrors, networkError }) {
        this.errors = graphQLErrors || networkError.result.errors;
  ...
  apollo: {
    summaryIEmp: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryIEmp;
          this.items = employee;
