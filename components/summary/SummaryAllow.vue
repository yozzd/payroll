<template>
  <div class="space-y-4">
    ...
    >
      ...
      <el-table-column label="Total" prop="tAllow" width="120" align="right">
        <template slot-scope="scope">
...
import { SummaryAllow } from '../../apollo/query/summary';
import { GenPDFSummaryAllow, GenXLSSummaryAllow } from '../../apollo/mutation/summary';
...
export default {
    ...
    return {
      ...
      miniSearch: new MiniSearch({
        ...
        storeFields: [
          ...
          'jul', 'agu', 'sep', 'okt', 'nov', 'des', 'tAllow',
        ],
      }),
    };
  },
  methods: {
    ...
    async genXLSSummary() {
      try {
        ...
        const y = this.$route.params.id;
        const { data: { genXLSSummaryAllow: { sStatus } } } = await this.$apollo.mutate({
          mutation: GenXLSSummaryAllow,
          variables: {
            id: parseInt(this.$route.params.id, 10),
          },
        });
  ...
  apollo: {
    summaryAllow: {
      ...
      result({ data, loading }) {
        if (!loading) {
          const { employee } = data.summaryAllow;
          this.items = employee;
